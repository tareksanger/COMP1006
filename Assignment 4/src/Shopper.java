...
public class Shopper {
    ...
    private int numItemsBeforeBags;
    // Constructor
    Shopper() {
    ...
    void addItem(GroceryBag g) {                                    // Add GroceryBag (Currently Not Accessed outside of Class)
        if (numItems < MAX_CART_ITEMS) {
            cart[numItems++] = g;
        }
    }
    // Adjusted from Marks A3 Solutions
    void removeItem(GroceryItem item) {                             // Remove GroceryItem   (Currently Not Accessed outside of Class)
        for (int i = 0; i < numItems; i++) {
            if (cart[i] == item) {
    ...
    void removeItem(GroceryBag item) {                              // Remove GroceryBag (Currently never used)
        for (int i = 0; i < numItems; i++) {
            if (cart[i] == item) {
                cart[i] = cart[numItems - 1];
                numItems--;
