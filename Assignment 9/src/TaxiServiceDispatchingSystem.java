...
public class TaxiServiceDispatchingSystem extends Application {
    ...
    private void update() {
        for (int i=0; i<areaLists.length; i++) {
            areaLists[i].setItems(FXCollections.observableArrayList(new ArrayList<Taxi>((Collection<? extends Taxi>) dispatchCenter.getAreas().get(DispatchCenter.AREA_NAMES[i]))));
            areaLists[i].refresh();
        }
